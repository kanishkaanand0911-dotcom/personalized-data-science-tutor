...
def explain_action(entry: ActionLogEntry) -> str:
    ...
    return f"On '{entry.column}', tried {entry.action_tried} — it {status}: {entry.reason}"


def explain_model(entry: ModelLogEntry) -> str:
    status = "was kept" if entry.passed else "was rejected"
    return f"Tried {entry.model_tried}, which {status}: {entry.reason}"
...
if __name__ == "__main__":
    ...
    key_status = "SET" if os.environ.get("ANTHROPIC_API_KEY") else "NOT SET — using deterministic fallback"
