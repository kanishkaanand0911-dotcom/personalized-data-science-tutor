...
def _evaluate_model(model, X, y, cv_folds: int = 5) -> dict:
    ...
    a residual-pattern check on a held-out fold, plus an overfitting check
    ...
    # Overfitting gap: how much better the model looks on data it already
    # saw vs data it didn't. A large positive gap means it memorized
    # training patterns that don't generalize -- this was completely
    # unchecked before; only cross-val/test scores existed, with no
    # train-vs-test comparison to catch a model that looks great on paper
    # but is really just memorizing.
...
OVERFIT_GAP_MAX = 0.25  # if train R^2 beats test R^2 by more than this, flag likely overfitting
...
def _passes_model_check(eval_result: dict) -> tuple[bool, str]:
    ...
    overfit_gap = eval_result.get("overfit_gap", 0)
    ...
    if overfit_gap > OVERFIT_GAP_MAX:
        return False, (f"training R^2 beats test R^2 by {overfit_gap} (above the {OVERFIT_GAP_MAX} limit) -- "
                        f"the model looks strong on data it already saw but doesn't generalize, a sign of overfitting")
    return True, (f"R^2={r2}, residuals look reasonably random (corr={resid_corr}), "
                   f"and train/test scores are close (gap={overfit_gap}) -- acceptable, generalizing fit")
...
def select_and_fit_model(df: pd.DataFrame, target_col: str, feature_cols: list,
    ...
    if observation["linearity_signal"] == "strong":
        ...
        candidates = [
            ("LinearRegression", LinearRegression(),
             "a straight-line model is the simplest option, and the data already shows a clear straight-line pattern",
             "can miss real patterns that aren't simple straight lines"),
            ("RandomForestRegressor", RandomForestRegressor(n_estimators=200, random_state=42),
             "a model that can bend and combine features in more complex ways, in case the simple pattern misses something",
             "harder to explain exactly why it made a specific prediction, and needs more data to work well"),
            ("GradientBoostingRegressor", GradientBoostingRegressor(random_state=42),
             "a model that builds up its predictions in stages, often catching subtle patterns simpler models miss",
             "slower to train and can overfit small datasets if not tuned carefully"),
        ]
    else:
        ...
        candidates = [
            ("LinearRegression", LinearRegression(),
             "starting with the simplest possible model gives us an honest baseline, even without an obvious straight-line pattern",
             "can miss real patterns that aren't simple straight lines"),
            ("Ridge", Ridge(alpha=1.0),
             "a variant of the simple model that's more cautious about overreacting to any single column",
             "still fundamentally a straight-line model, so it shares the same blind spot for complex patterns"),
            ("RandomForestRegressor", RandomForestRegressor(n_estimators=200, random_state=42),
             "a model that can pick up on combinations of columns mattering together, not just one at a time",
             "harder to explain exactly why it made a specific prediction, and needs more data to work well"),
            ("GradientBoostingRegressor", GradientBoostingRegressor(random_state=42),
             "a more powerful model that builds up its answer gradually, useful when simpler approaches fall short",
             "slower to train and can overfit small datasets if not tuned carefully"),
        ]
    ...
    for name, estimator, why_tried, tradeoff in candidates:
        ...
        entry = {
            "model_tried": name,
            "why_tried": why_tried,    # NEW: the reasoning behind trying this model, for teaching
            "tradeoff": tradeoff,      # NEW: this model's honest weakness, for pros/cons display
            "eval": eval_result,
            "passed": passed,
            "reason": reason,
        }
    ...
    return {
        ...
        "all_candidates_considered": [
            {"name": name, "why_tried": why, "tradeoff": tradeoff,
             "was_tried": any(e["model_tried"] == name for e in model_log)}
            for name, _est, why, tradeoff in candidates
        ],
    }
...
def predict_new_row(chosen_model: dict, input_values: dict) -> dict:
    """
    Lets a user try the fitted model on their own hypothetical input --
    "make predictions" (item 5 from the data scientist checklist). Not
    used anywhere in the automated pipeline; this exists for an interactive
    "try it yourself" UI feature.

    input_values: {column_name: value} for every feature the model expects.
    Missing columns get a type-aware default rather than crashing -- a user
    experimenting in a UI shouldn't have to fill in every single field.
    Numeric columns default to 0; categorical columns default to the first
    category the encoder actually learned during training (never a bare 0,
    which OneHotEncoder can't interpret as a category).

    Returns {"prediction": float, "used_defaults_for": [columns filled in automatically]}.
    """
    if chosen_model is None:
        return {"prediction": None, "used_defaults_for": [], "error": "no model was successfully fit"}
    ...
    for name, transformer, cols in prep.transformers_:
        ...
        for i, col in enumerate(cols):
            if col in input_values and input_values[col] not in (None, ""):
                row[col] = input_values[col]
                continue
            used_defaults.append(col)
            if name == "cat" and hasattr(transformer, "categories_"):
                row[col] = transformer.categories_[i][0]  # a real category the encoder learned, never a raw 0
            else:
                row[col] = 0  # numeric default; a real UI should pre-fill with the column's actual median

    input_df = pd.DataFrame([row])
    try:
        prediction = float(pipeline.predict(input_df)[0])
    except Exception as e:
        return {"prediction": None, "used_defaults_for": used_defaults, "error": str(e)}

    return {"prediction": round(prediction, 2), "used_defaults_for": used_defaults, "error": None}
